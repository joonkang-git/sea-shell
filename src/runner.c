...
get_io_flags(enum io_operator io_op)
{
  ...
  switch (io_op) {
    ...
    case OP_LESSAND: /* <& */
    case OP_LESS:    /* < */
      flags = O_RDONLY;     /* TODO - open(2) manpage*/
      break;
    case OP_GREATAND: /* >& */
    case OP_GREAT:    /* > */
      flags = O_WRONLY | O_EXCL | O_CREAT;      /* TODO */
      break;
    case OP_DGREAT: /* >> */
      flags = O_WRONLY | O_APPEND | O_CREAT;    /* TODO */
      break;
    case OP_LESSGREAT: /* <> */
      flags = O_RDWR | O_CREAT;       /* TODO */
      break;
    case OP_CLOBBER: /* >| */
      flags = O_WRONLY | O_TRUNC | O_CREAT;     /* TODO */
      break;
    // DONE
  }
  return flags;
...
move_fd(int src, int dst)
{
  ...
  /* TODO move src to dst - dup(2) Linux manpages*/
  dup2(src, dst);
  /* TODO close src */
  close(src);
  return dst;
...
run_command_list(struct command_list *cl)
{
  ...
  for (size_t i = 0; i < cl->command_count; ++i) {
    ...
    int pipe_fds[2] = {-1, -1};
    if (is_pl)  {
      // error handling for in pipeline error - Done
      if (pipe(pipe_fds) < 0) {
        return -1;
      }
    }
     ...
     */
    // remove const, it's messing with the variable assignments
    int did_fork = 0;
    if (!is_builtin || !is_fg) {
      child_pid = fork();
      //*Review this*/
    ...
    if (child_pid == 0) {
      if (is_builtin) {
         ...
         */
        // move upstream_pipfd to STDIN_FILENO if valid
